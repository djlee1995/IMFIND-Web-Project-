...
public class SocketController implements SessionName {
	...
	public ModelAndView socket(HttpServletRequest req, HttpSession ses, @ModelAttribute ChatDTO chatDTO, ModelAndView modelAndView) {
		
		String finder = chatDTO.getSender();
		int postnum =  chatDTO.getLost_postnum();
		String reciever = chatDTO.getReciever();
	
	    System.out.println("보내는 사람 " + finder); System.out.println("받는 사람 " + reciever);
