			 ...
			 $(document).on('click', '.p_lostlist_data', function(event){
				...
				$.ajax({
					...
					success: function(data){
						$('.police').empty();
						$('#output').empty();
						var place ='<br><p style="font-size: xx-large; font-weight:bold;"><a href=/imfind/p_lostlist.if class=p_lostlist_data id='+data[0].placeid+'>'+data[0].depplace+'</a></p> <p style="font-size: large;">'+data[0].addr+'<br>'+data[0].tel+'</p>';
						$('.police').append(place);
