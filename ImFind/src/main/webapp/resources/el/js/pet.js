		...
		.ready(
				function() {
					...
					jQuery('#petsido')
							.change(
									function() {
										...
										// 세종특별자치시 예외처리
										// 옵션값을 읽어 비교
										if (jQuery('#petsido option:selected')
												.val() == '36') {
											jQuery('#petsigugun').hide();
											// index를 이용해서 selected 속성(attr)추가
											// 기본 선택 옵션이 최상위로 index 0을 가짐
											jQuery('#petsigugun option:eq(1)')
													.attr('selected',
															'selected');
											// trigger를 이용해 change 실행
											jQuery('#petsigugun').trigger(
													'change');
										}
					...
					jQuery('#petsigugun')
							.change(
									function() {
										...
										// option의 맨앞에 추가
										jQuery('#petdong').prepend(
												fn_option('', '읍/면/동 선택하세요'));
										// option중 선택을 기본으로 선택
										jQuery('#petdong option:eq("")').attr(
												'selected', 'selected');
					...
					jQuery('#petdong').change(
							function() {
								var sido = jQuery('#petsido option:selected')
										.val();
								var sigugun = jQuery(
										'#petsigugun option:selected').val();
								var dong = jQuery('#petdong option:selected')
										.val();
